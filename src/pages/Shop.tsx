...
import { Search, Star, SlidersHorizontal, ChevronLeft, ChevronRight, X, Layers, DollarSign } from "lucide-react";
...
const Shop = () => {
  ...
  const filterSidebar = (
    <div className="space-y-8">
      <div>
        <h3 className="font-heading text-headline-sm text-on-surface mb-1">Filters</h3>
        <p className="font-body text-label-md text-on-surface-variant">Refine your selection</p>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-heading text-title-sm text-on-surface">Categories</h4>
        </div>
        <div className="space-y-2.5">
          {categories.filter(c => c !== "All").map((cat) => (
            <label key={cat} className="flex items-center gap-3 cursor-pointer group">
              ...
              />
              <span className={`font-body text-body-md transition-colors ${activeCategories.includes(cat) ? "text-on-surface font-medium" : "text-on-surface-variant group-hover:text-on-surface"}`}>
                {cat}
              </span>
            </label>
          ))}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-heading text-title-sm text-on-surface">Price Range</h4>
        </div>
        <input
          type="range"
          min={0}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-heading text-title-sm text-on-surface">Ratings</h4>
        </div>
        <div className="space-y-2.5">
          {[4, 3].map((r) => (
            <label key={r} className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="font-body text-body-md text-on-surface-variant flex items-center gap-1">
                {r}+ <Star className="w-3.5 h-3.5 text-primary fill-primary" /> stars
              </span>
            </label>
          ))}
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <h4 className="font-heading text-title-sm text-on-surface">Materials</h4>
        </div>
        <div className="flex flex-wrap gap-2">
          {materials.map((m) => (
            <span key={m} className="font-body text-label-md px-3 py-1 rounded-full bg-surface-container-high text-on-surface-variant">
              {m}
            </span>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-surface">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-body text-label-md text-on-surface-variant whitespace-nowrap">Sort By:</span>
                    <select
                      value={sortBy}
      ...
      {mobileFilters && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-on-surface/30" onClick={() => setMobileFilters(false)} />
          <div className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-surface-container-lowest p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-heading text-headline-sm text-on-surface">Filters</h3>
